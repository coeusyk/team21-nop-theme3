...
Use this verbatim when assigning work to Copilot:

> Build this project exactly according to `docs/PROJECT_ARCHITECTURE.md`, `docs/PLAN.md`, and `docs/TASK_LIST.md`. Implement a modular Python project for a numerical optimization course (Theme 3). Use `uv` for dependency management. Implement preprocessing for the House Prices Kaggle dataset, baselines (Ridge, standard LASSO), proximal operators, ISTA, FISTA, static adaptive LASSO, and dynamic reweighted LASSO. Use configurable hyperparameters $\lambda$, $\gamma$, $\epsilon$. Save metrics, runtime, objective traces, sparsity traces, tables, and plots per the required outputs list. Do not fabricate novelty claims; treat the dynamic reweighting as an application of IRL1 integrated into a proximal-gradient framework. Prefer readable, testable code over clever shortcuts.
