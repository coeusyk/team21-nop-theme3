"""Ridge regression baseline using sklearn RidgeCV.

Run as a script
---------------
    uv run python -m src.models.ridge --config configs/default.yaml
"""
...
def run_ridge(
    ...
    """Fit RidgeCV on training data and evaluate on validation and test splits.

    Uses leave-one-out cross-validation internally (``cv=None`` default of
    ``RidgeCV``) over the provided ``alphas`` grid to select the best
    regularisation strength.  All metrics are computed with
    :func:`~src.metrics.regression_metrics.compute_regression_metrics`.

    Parameters
    ----------
    X_train : np.ndarray, shape (n_train, p)
        Standardised training feature matrix from :func:`~src.data.split.build_dataset`.
    ...
    alphas : list[float]
        Regularisation strengths to search over via ``RidgeCV``.
    seed : int
        Random seed.  Set before fitting to ensure reproducibility.

    Returns
    -------
    dict with keys:
        ``beta``        — np.ndarray, shape (p,), fitted coefficients
        ``alpha_best``  — float, best alpha chosen by ``RidgeCV``
        ``val_mse``     — float
        ``val_mae``     — float
        ``test_mse``    — float
        ``test_mae``    — float
    ...
    nnz: int = int(np.sum(np.abs(beta) > 0))

    logger.info(
        "RidgeCV: alpha_best=%.4f  nnz=%d  val_mse=%.4f  test_mse=%.4f",
        alpha_best,
        nnz,
        val_metrics["mse"],
        test_metrics["mse"],
...
def _save_outputs(results: dict, cfg: dict, timestamp: str) -> None:
    """Save metrics CSV and coefficient CSV plus a JSON run log."""
    ...
    save_table(metrics_df, tables_dir, f"ridge_metrics_{timestamp}.csv")
    logger.info("Metrics saved to %s/ridge_metrics_%s.csv", tables_dir, timestamp)

    # --- Coefficient table ---
    coef_df = pd.DataFrame(
        {"feature": results["feature_names"], "coefficient": results["beta"]}
    ).sort_values("coefficient", key=np.abs, ascending=False)
    save_table(coef_df, tables_dir, f"ridge_coefficients_{timestamp}.csv")
    logger.info(
        "Coefficients saved to %s/ridge_coefficients_%s.csv", tables_dir, timestamp
    )

    # --- Run log ---
    log_payload = {
        "task": "T8-ridge-baseline",
        "timestamp": timestamp,
        "config": cfg,
        "metrics": {
            "alpha_best": results["alpha_best"],
    ...
    save_log(log_payload, logs_dir, f"ridge_run_{timestamp}.json")
    logger.info("Run log saved to %s/ridge_run_%s.json", logs_dir, timestamp)
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse command-line arguments for the Ridge script."""
    parser = argparse.ArgumentParser(
        description="T8 — Ridge regression baseline (RidgeCV)"
    )
    parser.add_argument(
        "--config",
        type=str,
...
def main(argv: list[str] | None = None) -> dict:
    """CLI entry point: load config, run Ridge, save outputs, return results dict."""
